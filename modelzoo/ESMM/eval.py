...
def train(sess_config,
    ...
    save_ckp_steps = save_steps if save_steps else train_steps
...
def eval(sess_config, input_hooks, model, test_init_op, test_steps, output_dir, checkpoint_dir, timeline_steps):
    hooks = []
    hooks.extend(input_hooks)

    model.global_step = tf.assign_add(model.global_step, 1)
    scaffold = tf.train.Scaffold(
        local_init_op=tf.group(tf.assign(model.global_step, 0),
                               tf.local_variables_initializer(), test_init_op))
    session_creator = tf.train.ChiefSessionCreator(
        scaffold=scaffold, checkpoint_dir=checkpoint_dir, config=sess_config)
    hooks.append(
        tf.train.StepCounterHook(every_n_steps=100))  # global steps/sec hook
    hooks.append(tf.train.StopAtStepHook(num_steps=test_steps))
    if timeline_steps > 0:
        hooks.append(
            tf.train.ProfilerHook(save_steps=timeline_steps,
                                  output_dir=os.path.join(
                                      checkpoint_dir, 'eval')))
    hooks.append(tf.train.LoggingTensorHook(
        {'steps': model.global_step}, every_n_iter=100))

    with tf.train.MonitoredSession(session_creator=session_creator,
                                   hooks=hooks) as sess:
        while not sess.should_stop():
            sess.run([model.acc_op, model.auc_op, model.global_step])
    print("Evalution complate.")
...
def main(stock_tf, tf_config=None, server=None):
    ...
    if args.steps == 0:
        ...
        test_steps = math.ceil(
            (float(no_epochs) * no_of_test_examples) / batch_size)
    else:
        no_epochs = math.ceil(
            (float(batch_size) * args.steps) / no_of_test_examples)
    ...
    train_dataset = build_model_input(train_file, batch_size, 1, SEED, stock_tf, args.workqueue)
    test_dataset = build_model_input(test_file, batch_size, no_epochs, SEED, stock_tf, args.workqueue)
    ...
    if not (args.no_train or tf_config):
    ...
    eval(sess_config, hooks, model, test_init_op, test_steps,
            model_dir, checkpoint_dir, args.timeline)
