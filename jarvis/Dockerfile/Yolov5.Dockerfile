...
RUN sudo ldconfig
# PyTorch
RUN /usr/local/bin/pip3 install --no-cache-dir torch==1.11.0
RUN /usr/local/bin/pip3 install --no-cache-dir torchvision==0.12.0
RUN /usr/local/bin/pip3 install intel-extension-for-pytorch==1.11.200