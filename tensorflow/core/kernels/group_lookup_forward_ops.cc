      ...
      auto do_lookup = [this, ctx, embedding_var, unique, default_v,
        ...
        for (int k = start; k < end; ++k) {
          ...
          OP_REQUIRES_OK(ctx, lookup_fn_(embedding_var, unique[k],
                                         ...
                                         /*sp_weights,*/ default_v_ptr, count));
        }
      };
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    auto do_compute = [this, ctx, batch_size, worker_threads](int i) {
        ...
        TensorShape emb_vectors_tensor_shape = TensorShape(std::vector<int64>(
            {static_cast<long long>(batch_size), dimension_}));
        ...
        if (combiner_ == "mean") {
          auto do_var_mean = [this, &emb_vectors, batch_nums, unique_idx,
                              unique, sp_weights,
          ...
          auto do_var_sqrtn = [this, &emb_vectors, batch_nums, unique_idx,
                               unique, sp_weights,
                               embedding_variable](int64 start, int64 end) {
            for (int64 i = start; i < end; ++i) {
              std::vector<TValue> tmp_embedding(dimension_, 0.0f);
